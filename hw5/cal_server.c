...
int main(int argc, char **argv)
{
		...
		printf("%s %s \n", rdata.stu_num, rdata.stu_name);
		printf("%s : ", rdata.stu_num);
		for(int i=0; i<20; i++){
			printf("%02X ", (rdata.stu_num[i])&0x000000FF);
		}
		printf("\n");
		printf("%s : ", rdata.stu_name);
		for(int i=0; i<20; i++){
			printf("%02X ", (rdata.stu_name[i])&0x000000FF);
		}
		printf("\n");
		for(int i=0; i<4; i++){ //피연산자 출력
			printf("%02X ", (rdata.left_num >> i*8)&0x000000FF);
		}

		for(int i=0; i<4; i++){ //피연산자 출력
			printf("%02X ", (rdata.right_num >> i*8)&0x000000FF);
		}

		//연산자 출력
		printf("%02X ", rdata.op&0x000000FF);

                cal_result = 0;
                cal_error = 0;

		for(int i=0; i<4; i++){ //결과 출력
			printf("%02X ", (rdata.result >> i*8)&0x000000FF);
		}
		for(int i=0; i<4; i++){ //에러 코드 출력
			printf("%02X ", (rdata.error >> i*8)&0x000000FF);
		}
